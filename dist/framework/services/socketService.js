...
const initSocketsever = (server) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('stop typing', ({ user, receiver }) => socket.in(receiver).emit('stop typing', { user }));
        socket.on('newchatwithuser', (userId) => socket.in(userId).emit('newchat comming'));
        //like post
        ...
        socket.on('new comment', ({ response, postId, postOwnerId, reply }) => {
            if (reply !== '') {
                if (response.author._id == reply)
                    return;
                socket
                    .in(reply)
                    .emit('reply comment', { response, postOwnerId, reply, postId });
            }
            if (response.author._id == postOwnerId)
                return;
            else {
                socket
                    .in(postOwnerId)
